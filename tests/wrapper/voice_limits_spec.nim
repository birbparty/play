...
spec "SoLoud voice limit policy":
  ...
  it "rejects invalid or late voice limit configuration":
    given:
      let engine = newEngine()
  ...
  it "bounds active mixed voices under low voice limit":
    given:
      let engine = newEngine()
    ...
    act:
      configResult = engine.setVoiceOptions(voiceOptions(maxActiveVoices = 1'u32))
      initResult = engine.init(initOptions(backend = nullBackend))
